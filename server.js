...
io.on('connection', function (socket) {
    ...
    socket.on('updatePlayer', function (playerObject) {
        ...
        if (loggedIn) {
            ...
            if(Math.abs(playerObject.x - thisPlayerObject.x) > 10 || playerObject.x > mapSize[0] || playerObject.x < 0){
                playerObject.x = thisPlayerObject.x;
                movementInvalid = true;
            }
            if(Math.abs(playerObject.y - thisPlayerObject.y) > 10 || playerObject.y > mapSize[1] || playerObject.y < 0){
